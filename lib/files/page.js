...
const nunjucksHelpers = require('../helpers-nunjucks')
...
const nunjucksPath = new nunjucks.FileSystemLoader(process.cwd())
...
module.exports = class Page extends File {
  ...
  render(context, done) {
    ...
    if (layout && templateEngine === 'handlebars') {
      ...
      var nunjucksEnv = new nunjucks.Environment(nunjucksPath)
      nunjucksEnv = nunjucksHelpers.transformEnv(nunjucksEnv)

      output = nunjucksEnv.renderString(layout.input, ctx)
    } else {
